...
class UpdateInterstitialRequest extends FormRequest
{
    ...
    {
        $interstitial = $this->route('interstitial');
        $tableName = config('larastitial.tables.interstitials', 'interstitials');

        return [
            'name' => ['required', 'string', 'max:255', Rule::unique($tableName)->ignore($interstitial->id)],
